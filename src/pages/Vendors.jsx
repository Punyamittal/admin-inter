...
import { Users, Plus, Search, Filter, Mail, Phone, MapPin, MoreVertical, Loader2, UserCheck, X, Eye, EyeOff, Edit2, Trash2 } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        if (editingVendor) {
            ...
            toast.success('New vendor activated.');
        }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(15, 23, 42, 0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1200 }}>
           <div className="card" style={{ width: '90%', maxWidth: '480px', padding: '32px' }}>
              ...
              <h2 style={{ fontSize: '20px', marginBottom: '8px', color: 'var(--primary)' }}>{editingVendor ? 'Edit Partner' : 'Vendor Onboarding'}</h2>
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                 <input type="text" required value={formData.fullName} onChange={(e) => setFormData({...formData, fullName: e.target.value})} placeholder="Full Name *" style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid var(--border)' }} />
                 <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                    <input type="email" disabled={!!editingVendor} required={!editingVendor} value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="Email" style={{ flex: '1 1 200px', padding: '12px', borderRadius: '10px', border: '1px solid var(--border)', backgroundColor: editingVendor ? '#F8FAFC' : 'white' }} />
                    <input type="tel" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} placeholder="Phone" style={{ flex: '1 1 120px', padding: '12px', borderRadius: '10px', border: '1px solid var(--border)' }} />
                 </div>
                 {!editingVendor && (
                   <div style={{ position: 'relative' }}>
                        <input type={showPassword ? 'text' : 'password'} required minLength={8} value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} placeholder="Password *" style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid var(--border)' }} />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', right: '12px', top: '12px', border: 'none', background: 'none' }}>{showPassword ? <EyeOff size={18} color="var(--text-muted)" /> : <Eye size={18} color="var(--text-muted)" />}</button>
                   </div>
                 )}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
        <div><h1 style={{ fontSize: '24px', color: 'var(--primary)' }}>Authorized Vendors</h1><p style={{ color: 'var(--text-muted)' }}>Secure partner management.</p></div>
      ...
      <div className="card" style={{ padding: '0' }}>
        ...
        <div style={{ padding: '20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
          {filteredVendors.map((vendor) => (
            <div key={vendor.id} className="card" style={{ padding: '20px', border: '1px solid var(--border)', position: 'relative' }}>
